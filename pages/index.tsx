  ...
  export default function Home({ data }:any) {
    ...
    const handleSubmit = async (e:any) => {
      ...
      for (let i = 0; i < data.length; i++) {
        if (data[i][8].trim().toLowerCase() === input.toLowerCase()) { //I
          setMatch(true);
          redirect_url += input;
  ...
  */}
        ...
        <div className="container w-full max-w-xs bg-white bg-opacity-50 mx-auto shadow-md">
          <form className="justify-center items-center rounded px-8 pt-6 pb-4" onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                <label className=" block text-orange-600 text-lg font-bold pb-2">
                请输入报名邮箱
                </label>
                <label className=" block text-orange-600 text-sm font-bold mb-2 pb-2">
        ...
        <footer className="w-screen text-center mt-6">
          <span className="text-orange-600 text-xs">&copy; <strong>UTCSSA</strong> - Junyu Yao and Tech Department , 2024.</span>
        </footer>
